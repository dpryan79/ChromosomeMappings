...
It's often the case that a contig will have an associated version, such as `.1` in `GL877872.1`. While the contig itself will exist across genome updates, the version number may change. Consequently, it may be required to strip off these version when performing name conversions, simply to support different versions/patches of the same genome.
